...
export class PersonTypeormRepository implements IPersonRepository {
  ...
  async update(entity: Person): PersonUpdate.Output {
    ...
    const personUpdateResult = await this.personRepo.update(entity.id, {
      id: entity.id,
      cpf: entity.cpf,
  ...
  async getActiveRecords(): Promise<Person[]> {
    const models = await this.personRepo
  ...
  async _get(id: string) {
    ...
    return new Person({
      ...
      userAttr: {
        ...person.user,
        profile_picture: {
          id: person.user.profile_picture,
          url: profile_picture.url,
        },
        header_picture: {
          id: person.user.header_picture,

...
export class UserTypeormRepository implements IUserRepository {
  ...
  async findUserById(id: string): UserFindById.Output {
    ...
    if (user.role === Role.PERSON) return this.RepoPerson.findById(user.id);

    if (user.role === Role.SHELTER) return this.RepoShelter.findById(user.id);

    if (user.role === Role.NGO) return this.RepoNGO.findById(user.id);
  ...
  async resetPassword(user: User): Promise<User> {
    const userUpdateResult = await this.repo.update(user.id, {
      password: user.password,
    });

    if (userUpdateResult.affected === 0) return null;

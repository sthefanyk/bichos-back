...
export namespace ShelterUpdate {
  export class Usecase implements UseCase<Input, Output> {
    ...
    async execute(input: Input): Output {
      ...
      if (!result)
        throw new UpdateError(`Could not update shelter with ID ${shelter.id}`);

      return result;
    }

    async validate(input: Input) {
      if (!input.responsible_date_birth)
        throw new RequiredError('responsible_date_birth');
      ...
      if (!input.star_date_shelter)
        throw new RequiredError('star_date_shelter');
      ...
      const nameShelterExists = await this.repo.findByNameShelter(
        input.name_shelter.toLowerCase(),
      );
      ...
      if (nameShelterExists && nameShelterExists.id !== id)
        throw new AlreadyExistsError('Name shelter already exists');
      if (usernameExists && usernameExists.id !== id)
        throw new AlreadyExistsError('Username already exists');
      if (cpfExists && cpfExists.id !== id)
        throw new AlreadyExistsError('CPF already exists');
    }
  }
